...
public interface UserApi
{
  ...
  ResponseEntity<Long> save(@RequestBody UserCreateDto userCreateDto);

  @GetMapping(value = "/users/{id}")
  ResponseEntity<UserDto> get(@PathVariable(name = "id") Long id);

  @GetMapping(value = "/users")
  ResponseEntity<PageDto<UserDto>> getAll(@RequestParam(defaultValue = "0") int page, @RequestParam(defaultValue = "100") int size);
}
